...
const makeMessagesSocket = (config) => {
    ...
    const felz = newsletter_1.makeNewsletterSocket(config)
    const {
       ev, 
       authState, 
       processingMutex, 
    ...
    const profilePictureUrl = async (jid) => {
        if (WABinary_1.isJidNewsletter(jid)) {
         ...
         let directPath = metadata.thread_metadata.picture?.direct_path
        return directPath ? Utils_1.getUrlFromDirectPath(directPath) : null
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, useUserDevicesCache, useCachedGroupMetadata, statusJidList, additionalNodes }) => {
        ...
        msgId = msgId || config.customMessageID() || Utils_1.generateMessageID()
        ...
        const commentMessage = messages.commentMessage || messages.encCommentMessage
        ...
        await authState.keys.transaction(async () => {
           ...
           if(!messages.reactionMessage && (isPerson || isLid)) {
				       const nodes = WABinary_1.getAdditionalNode('bot')
               const filteredBotNode = WABinary_1.getBinaryNodeFilter(additionalNodes)
               if (filteredBotNode) {
                  didPushAdditional = true
                  stanza.content.push(...additionalNodes)
               }
           ...
           if(!messages.reactionMessage && !additionalNodes && !isStatus && !isNewsletter && !buttonType) {
                 const defaultBizNode = WABinary_1.getAdditionalNode()
                 const filtered = WABinary_1.getBinaryNodeFilter(additionalNodes)
                 if (filtered) {
                    didPushAdditional = true
                    stanza.content.push(...additionalNodes)
                 } 
                 else {
                    stanza.content.push(...defaultBizNode)
                 }
           }
    ...
    const getButtonType = (msg) => {
		  if (msg.buttonsMessage) {
			  return 'buttons'
	  	} else if (msg.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name === 'review_and_pay' || msg.templateMessage?.interactiveMessageTemplate?.nativeFlowMessage?.buttons?.[0]?.name === 'review_and_pay') {
        return 'review_and_pay'
      } else if (msg.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name === 'review_order' || msg.templateMessage?.interactiveMessageTemplate?.nativeFlowMessage?.buttons?.[0]?.name === 'review_order') {
        return 'review_order'
      } else if (msg.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name === 'payment_info' || msg.templateMessage?.interactiveMessageTemplate?.nativeFlowMessage?.buttons?.[0]?.name === 'payment_info') {
        return 'payment_info'
		  } else if (msg.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name === 'payment_status' || msg.templateMessage?.interactiveMessageTemplate?.nativeFlowMessage?.buttons?.[0]?.name === 'payment_status') {
        return 'payment_status'
		  } else if (msg.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name === 'payment_method' || msg.templateMessage?.interactiveMessageTemplate?.nativeFlowMessage?.buttons?.[0]?.name === 'payment_method') {
        return 'payment_method'
	  	} else if (msg.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name === 'cta_catalog' || msg.templateMessage?.interactiveMessageTemplate?.nativeFlowMessage?.buttons?.[0]?.name === 'cta_catalog') {
        return 'cta_catalog'
		  } else if (msg.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name === 'mpm' || msg.templateMessage?.interactiveMessageTemplate?.nativeFlowMessage?.buttons?.[0]?.name === 'mpm') {
        return 'mpm'
		  } else if (msg.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name === 'call_permission_request' || msg.templateMessage?.interactiveMessageTemplate?.nativeFlowMessage?.buttons?.[0]?.name === 'call_permission_request') {
        return 'call_request'
      } else if (msg.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name === 'automated_greeting_message_view_catalog' || msg.templateMessage?.interactiveMessageTemplate?.nativeFlowMessage?.buttons?.[0]?.name === 'automated_greeting_message_view_catalog') {
        return 'view_catalog'
		  } else if (msg.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name === 'wa_payment_transaction_details' || msg.templateMessage?.interactiveMessageTemplate?.nativeFlowMessage?.buttons?.[0]?.name === 'wa_payment_transaction_details') {
        return 'wa_pay_detail'
	  	} else if (msg.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name === 'send_location' || msg.templateMessage?.interactiveMessageTemplate?.nativeFlowMessage?.buttons?.[0]?.name === 'send_location') {
        return 'send_location'
		  } else if (msg.interactiveMessage && msg.interactiveMessage?.nativeFlowMessage) {
			  return 'interactive'
	  	} else if (msg.templateMessage?.interactiveMessageTemplate) {
	  	  return 'templateInteractive'
    ...
    const getEphemeralGroup = async (jid) => {
       ...
       let results = WABinary_1.getBinaryNodeChild(metadata, 'group')
       let eph = WABinary_1.getBinaryNodeChild(results, 'ephemeral')
       return eph?.attrs?.expiration || 0
	  }
	  
	  const fetchEphemeralMessage = async (jid) => {
	     let expiration
       if (WABinary_1.isJidGroup(jid)) {
         ...
         if (disappearing.length) {
           const finder = disappearing.find(e => e.disappearing_mode)
           if (finder) expiration = finder.disappearing_mode?.duration || 0
         }
       }
       else expiration = 0
       return expiration
    ...
    return {
        ...
        sendStatusMentions: async (content, jids = []) => {
            ...
            for (const id of jids) {
		            const { user, server } = WABinary_1.jidDecode(id)
		            const isGroup = server === 'g.us'
                const isPerson = server === 's.whatsapp.net'
                if (isPerson) {
                   let personaJid = jids.filter(jid => jid.endsWith('s.whatsapp.net'))
                   allUsers = [...allUsers, ...personaJid]
                }
                if (isGroup) {
                   let groupJids = jids.filter(jid => jid.endsWith('g.us'))
                    for (const groupId of groupJids) {             
                        let metadata = await felz.groupMetadata(groupId)
                        let participants = await metadata.participants
                        let memberJid = participants.map(
                            b => WABinary_1.jidNormalizedUser(b.id)
                        )
            ...
            let mediaHandle;
            let msg = await Utils_1.generateWAMessage(
                WABinary_1.STORIES_JID, 
                content, 
                {
				            logger,
                    userJid,
                    ...
                    upload: async (encFilePath, opts) => {
                        const up = await waUploadToServer(encFilePath, { ...opts, newsletter: false })
            ...
            jids.forEach(async id => {
               id = WABinary_1.jidNormalizedUser(id)
		           const { user, server } = WABinary_1.jidDecode(id)
               const isGroup = server === 'g.us'
               let content = isGroup
               ...
               let type = isGroup 
                   ...
                   : 'is_status_mention'
               const statusMentionMsg = await Utils_1.generateWAMessageFromContent(
                   id,
                   {
                       messageContextInfo: {
                       ...
                       [content]: {
                           message: {
                               protocolMessage: {
                                   ...
                                   type: 25,
                                   ephemeralExpiration: await fetchEphemeralMessage(id),
                                   disappearingMode: {
			                                initiator: 0,
			                                trigger: 2,
			                                initiatedByMe: false 
               ...
               await relayMessage(id, statusMentionMsg.message,
               {
                  messageId: config.customMessageID() || Utils_1.generateMessageID(),
                  additionalNodes: [{
                     tag: 'meta',
                     attrs: { [type]: 'true' }
                  }]
        ...
        sendAlbumMessage: async (jid, medias, options = {}) => {
            ...
            const eph = await fetchEphemeralMessage(jid)
            ...
            }, { userJid, ephemeralExpiration: eph, ...options })
