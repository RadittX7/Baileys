...
const makeMessagesRecvSocket = (config) => {
    ...
    const felz = messages_send_1.makeMessagesSocket(config)
    const { ev, authState, ws, processingMutex, signalRepository, query, upsertMessage, resyncAppState, onUnexpectedError, assertSessions, sendNode, relayMessage, sendReceipt, uploadPreKeys, groupMetadata, getUSyncDevices, createParticipantNodes, sendPeerDataOperationMessage } = felz
    ...
    const handleGroupNotification = (participant, child, msg) => {
        const participantJid = WABinary_1.isLidUser(WABinary_1.getBinaryNodeChild(child, 'participant')?.attrs?.jid) ? (WABinary_1.getBinaryNodeChild(child, 'participant')?.attrs?.phone_number || participant) : getBinaryNodeChild(child, 'participant')?.attrs?.jid || participant
                        ...
                        if (WABinary_1.isJidGroup(remoteJid) || WABinary_1.isJidStatusBroadcast(remoteJid)) {
                            if (attrs.participant) {
                                ...
                                ev.emit('message-receipt.update', ids.map(id => ({
                                    key: { ...key, id },
                                    receipt: {
                                        userJid: WABinary_1.jidNormalizedUser(participant),
                                        [updateKey]: +attrs.t
                                    }
                                })))
                            }
                ...
                processingMutex.mutex(async () => {
                    ...
                    if (msg) {
                        ...
                        msg.key = {
                            remoteJid,
                            fromMe,
                            participant: fromMe ? WABinary_1.jidNormalizedUser(authState.creds.me.id) : participant,
                            id: node.attrs.id,
                            ...(msg.key || {})
                        }
                        msg.participant = fromMe ? WABinary_1.jidNormalizedUser(authState.creds.me.id) : participant
                ...
                processingMutex.mutex(async () => {
                    ...
                    if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT) {
                        ...
                        const content = Utils_1.normalizeMessageContent(msg.message)
	                      const key = Utils_1.getContentType(content)
	                      const message = content[key]
	                      const contextInfo = message.contextInfo
              
	                      if (node.attrs.addressing_mode === 'lid') {
	                          const metadata = WABinary_1.isJidGroup(node.attrs.from) ? await groupMetadata(node.attrs.from) : null
                            let found = metadata ? metadata.participants.find(p => p.lid === node.attrs.participant) : null

                            msg.key.participant = found ? found.id : participant
                            participant = found ? found.id : participant
                            
	                          if (content && contextInfo && contextInfo.participant && WABinary_1.isLidUser(contextInfo.participant || node.attrs.participant)) {
	                             if (WABinary_1.isJidGroup(content && contextInfo && contextInfo.remoteJid ? contextInfo.remoteJid : node.attrs.from) && isLidUser(contextInfo.participant ?? node.attrs.participant)) {
	                                ...
	                                contextInfo.participant = WABinary_1.jidNormalizedUser(result?.id)
	                             }
	                          }
	            
	                          if (content && contextInfo && contextInfo.mentionedJid) {
	                             if (WABinary_1.isJidGroup(contextInfo.remoteJid || node.attrs.from)) {
	                                 let mentions = []
	                                 for (const ids of contextInfo.mentionedJid) {
	                                     if (WABinary_1.isLidUser(ids)) {
	                                         const { participants } = await groupMetadata(contextInfo.remoteJid || node.attrs.from)
	                                         const group = participants ? participants.find(p => p?.lid === ids) : null
                             
	                                         if (group) {
	                                             mentions.push(WABinary_1.jidNormalizedUser(group?.id))
	                                         } else {
	                                             mentions.push(ids)
	                                         }
	                                     }
	                             ...
	                             } else if (!WABinary_1.isJidGroup(contextInfo.remoteJid || node.attrs.from) && WABinary_1.isLidUser(content && contextInfo && contextInfo.remoteJid ? contextInfo.remoteJid : node.attrs.from)) {
	                                 const sender_pn = WABinary_1.jidNormalizedUser(node.attrs.peer_recipient_pn || node.attrs.sender_pn || node.attrs.recipient || node.attrs.participant_pn || node.attrs.participant)
