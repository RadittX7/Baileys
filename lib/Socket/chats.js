...
const makeChatsSocket = (config) => {
    ...
    const felz = usync_1.makeUSyncSocket(config)
    const { ev, ws, authState, generateMessageTag, sendNode, query, onUnexpectedError, groupFetchAllParticipating } = felz
    let privacySettings
    ...
    const updateDefaultDisappearingMode = async (duration) => {
	  ...
	  const getBotListV2 = async (categoryName) => {
		...
		const botList = []
		const listview = []
		const metaia = WABinary_1.getBinaryNodeChild(botNode, 'default')
		botList.push({
		  category: 'default',
		  listview: [{
		     name: 'Meta AI',
  	...
  	const getBotListV3 = async (categoryName) => {
	  if(!categoryName) {
	    throw new boom_1.Boom('Illegal no-category name. Please specify either category name', { statusCode: 500 })
	  }
    ...
    const getBusinessCoverPhoto = async (jid) => {
        const results = await query({
		        tag: 'iq',
		        attrs: {
			         to: 's.whatsapp.net',
        ...
        const profileNode = WABinary_1.getBinaryNodeChild(results, 'business_profile')
	      const profiles = WABinary_1.getBinaryNodeChild(profileNode, 'profile')
        const cover = WABinary_1.getBinaryNodeChild(profiles, 'cover_photo')
