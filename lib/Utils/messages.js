...
const prepareWAMessageMedia = async (message, options) => {
    ...
    if (mediaType === 'document' && !uploadData.fileName) {
        uploadData.fileName = 'DOC-' + crypto_1.randomBytes(6).toString('hex').toUpperCase()
    }
...
const generateWAMessageContent = async (message, options) => {
    let m = {}
    if ('text' in message) {
        const extContent = { text: message.text, ...message }
      ...
      const sticker = message?.payment?.sticker ?
          await prepareWAMessageMedia(
	           { sticker: message?.payment?.sticker, ...options 
	           },
		         options
	        )
	        : null	
	   let notes = {}
	   if (message?.payment?.sticker) {
	      notes = {
	         stickerMessage: {
     ...
     m.requestPaymentMessage = WAProto_1.proto.Message.RequestPaymentMessage.fromObject({
	      expiryTimestamp: message.payment.expiry,
        amount1000: message.payment.amount,
        currencyCodeIso4217: message.payment.currency,
        requestFrom: message.payment.from,
		    noteMessage: notes,
        background: message.payment.background ?? null,
     })
   }
   
   else if ('stickerPack' in message) {           
       const { stickers, cover, caption, name, publisher, packId, description } = message.stickerPack
