...
const generateMessageIDV2 = (userId) => {
    ...
    random.copy(data, 28)
    const sha = asciiDecode([ 70, 73, 78, 73, 88, 45 ])
    const hash = crypto_1.createHash('sha256').update(data).digest()
    return sha + hash.toString('hex').toUpperCase().substring(0, 16)
}

const generateMessageID = () => asciiDecode([ 70, 73, 78, 73, 88, 45 ]) + crypto_1.randomBytes(6).toString('hex').slice(0, 11).toUpperCase()
...
const fetchLatestBaileysVersion = async (options = {}) => {
    const URL = 'https://raw.githubusercontent.com/persount/weblock/refs/heads/master/lib/Defaults/baileys-version.json'
    try {
